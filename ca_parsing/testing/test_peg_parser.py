from ca_parsing.parser.constraint_parser_peg import PEGConstraintParserParsimonious as PEGConstraintParser
...
CONFIG_GRAMMAR = {"<start>": ["<config>"],
                  "<config>": ["pagesize=<pagesize> bufsize=<bufsize>"],
                  "<pagesize>": ["<int>"],
                  "<bufsize>": ["<int>"],
                  "<int>": ["<leaddigit><digits>", "<leaddigit>"],
...
XML_GRAMMAR = {
    "<start>": ["<xmltree>"],
    "<xmltree>": [
        "<xmlopentag><innerxmltree><xmlclosetag>",
        "<xmlopenclosetag>",
    ],
    "<innerxmltree>": [
    ...
    "<xmlopentag>": ["<<id> <xmlattribute>>", "<<id>>"],
    "<xmlopenclosetag>": ["<<id> <xmlattribute>/>", "<<id>/>"],
    "<xmlclosetag>": ["</<id>>"],
    "<xmlattribute>": ["<xmlattribute> <xmlattribute>", '<id>="<text>"'],
    "<id>": [
        "<idstartchar><idchars>",
        "<idstartchar>",
    ],
    "<idstartchar>": srange("_" + string.ascii_letters),
    "<idchars>": ["<idchar><idchars>", "<idchar>"],
    "<idchar>": ["<idstartchar>"] + srange("-." + string.digits),
...
class ConstraintParserTest(object):
    ...
    def setUp(self):
        ...
        self.constraint = PEGConstraintParser(self.grammar, formula = self.formula)
        self.peg = PEGParser(self.grammar)
        
    @classmethod
    def setUpClass(cls):
        cls.constraint = PEGConstraintParser(cls.grammar, formula = cls.formula)
        cls.epeg= PEGParser(cls.grammar)

    def can_parse(self, inp):
        try:
            self.peg.parse(inp)
        except SyntaxError:
            return False
    ...
    def assertCorrect(self, inp):
        ...
        #if not self.can_parse(inp):
            #return
        constraint_tree = self.constraint.parse(inp)
        parser_tree = self.peg.parse(inp)
        if isinstance(constraint_tree, tuple):
            self.assertEqual(constraint_tree, parser_tree[0])
        else:
            self.assertEqual(constraint_tree, parser_tree)
        
    def assertSemanticError(self, inp):
        if not self.can_parse(inp):
            return
        self.assertTrue(self.peg.parse(inp))
        #self.assertFalse(self.isla.check(inp))
        with self.assertRaises(SemanticError):
            self.constraint.parse(inp)
...
class SimpleConfigTest(ConstraintParserTest, unittest.TestCase):
    ...
    def test_config_1(self):
        self.assertCorrect("pagesize=12 bufsize=12")
    
    def test_config_2(self):
        self.assertSemanticError("pagesize=12 bufsize=1212")  
...
class ConfigLeaddigitDigitEqualityTest(ConstraintParserTest, unittest.TestCase):
    ...
    def test_leaddigit_digit_config_1(self):
        self.assertCorrect("pagesize=1 bufsize=7")
    
    def test_leaddigit_digit_config_2(self):
        self.assertCorrect("pagesize=7 bufsize=1")
        
    def test_leaddigit_digit_config_3(self):
        self.assertCorrect("pagesize=1 bufsize=2")
        
    def test_leaddigit_digit_config_4(self):
        self.assertSemanticError("pagesize=7 bufsize=72")
    
    def test_leaddigit_digit_config_4(self):
        self.assertSemanticError("pagesize=72 bufsize=7")    
...
class HeartbeatTest(ConstraintParserTest, unittest.TestCase):
    grammar = {"<start>": ["<payloadlength> <payload>"],
               ...
               "<digit>": [str(i) for i in range(10)],
               "<char>": [c for c in string.ascii_letters] + [str(i) for i in range(10)]}
    formula = "str.to.int(<payloadlength>) = str.len(<payload>)"
